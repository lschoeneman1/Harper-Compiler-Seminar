...
BooleanExpression* Parser::parseBooleanExpression()
{
	...
	{
		string err = format("Expected comparison operator (==, !=, <, >, <=, >=) at line {}, column {}", peek().line,peek().column);
...
Expression* Parser::parseFactor()
{
	...
	string err = format("Unexpected token: {} at line {}, column {}",static_cast<int>(peek().type),peek().line,peek().column);
	throw runtime_error(err);
}
...
Token Parser::consume(TokenType type, const string& message)
{
	...
	Token token = peek();
	string err = format("{} at line {}, column {}",  message, token.line, token.column);
	throw runtime_error(err);
}
